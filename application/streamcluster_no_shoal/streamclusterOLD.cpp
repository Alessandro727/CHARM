...
void localSearch( Points* points, long kmin, long kmax, long* kfinal ) {
...
#ifdef ENABLE_THREADS
      Charm::all_do([&threads, &i, &arg]{
      pthread_create(threads+i,NULL,localSearchSub,(void*)&arg[i]);
      });
      // std::cout << "Start: " << i << std::endl;
      // futuresCharm.push_back(Charm::call<Charm::async>(i, [&i, &arg](){
    ...
    std::cout << "All call done!" << std::endl;
#endif
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  while(1) {
    ...
    for( int i = 0; i < 8; i++ ) {      
      Charm::call<Charm::async>(i, [&i, &points,&kmin, &kmax,&kfinal](){
    ...
    std::cout << "concenters done!" << std::endl;
    ...
    std::cout << "copycenters done!" << std::endl;
    IDoffset += numRead;
