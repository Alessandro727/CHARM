...
int charmCallStreamCluster(int i, pkmedian_arg_t* arg) {
  Charm::call<Charm::async>(i, [&i, &arg](){
    localSearchSub(&arg[i]);
              return 1;
        });//call
}
...
void localSearch( Points* points, long kmin, long kmax, long* kfinal ) {
    ...
    std::vector<std::future<int>> futuresInt;
    // std::vector<std::future<void*>> futuresInt;
...
futuresInt.push_back(std::async(std::launch::async, charmCallStreamCluster, i, arg));
// futuresInt.push_back(std::async(std::launch::async, localSearchSub,(void*)&arg[i]));
// futuresCharm.push_back(Charm::call<Charm::async>(i, [&i, &arg]{std::async(std::launch::async, localSearchSub, (void*)&arg[i]); return 1;}));
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
 ...
 Charm::run([&stream, &kmin, &kmax, &dim, &chunksize, &clustersize, &outfilename]{
