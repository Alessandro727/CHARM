...
struct bucket_t {
  ...
  void reserve(size_t nelems) {
    ...
    if (!v) {
      LOG_CHARM_LEVEL(INFO) << "Unable to allocate bucket of " << nelems << " elements."; exit(1);
    }
  }
...
inline uint64_t next_random() {
  ...
  static engine_t engine(12345L*my_id());
...
void bucket_sort(GlobalAddress<uint64_t> array, size_t nelems, size_t nbuckets) {
  ...
  GlobalAddress<bucket_t> bucketlist = gmalloc<bucket_t>(nbuckets);

#ifdef DEBUG
  for (size_t i=0; i<nbuckets; i++) {
    GlobalAddress<bucket_t> bi = bucketlist+i;
    LOG_CHARM_LEVEL(1) << "bucket[" << i << "] on Core " << bi.core() << ", offset = " << bi.pointer() - bucketlist.localize(bi.core()) << ", ptr = " << bi.pointer();
  }
#endif
  ...
  pfor(array, nelems, [](int64_t i, uint64_t& v){
    size_t b = v >> LOBITS;
    counts[b]++;
  });

  histogram_time = walltime() - t;
  LOG_CHARM_LEVEL(INFO) << "histogram_time: " << histogram_time;
  t = walltime();
  ...
  all_do([nbuckets]{
    ...
    allreduce<size_t,collective_add>(counts.size());
    LOG_CHARM_LEVEL(1) << "after allreduce_inplace (just in case)";
  ...
  allreduce_time = walltime() - t;
  LOG_CHARM_LEVEL(INFO) << "allreduce_time: " << allreduce_time;

  // allocate space in buckets
  LOG_CHARM_LEVEL(3) << "allocating space...";
  // forall_local<bucket_t,init_buckets>(bucketlist, nbuckets);
  pfor(bucketlist, nbuckets, [](int64_t id, bucket_t& bucket){
    // (global malloc doesn't call constructors)
    new (&bucket) bucket_t();
    bucket.reserve(counts[id]);
  });
  
  LOG_CHARM_LEVEL(3) << "scattering...";
      t = walltime();

  // scatter into buckets
  // forall_local<uint64_t,scatter>(array, nelems);
  pfor(array, nelems, [bucketlist](int64_t s, int64_t n, uint64_t * first){
    ...
    for (int i=0; i<n; i++) {
      ...
      call<async>(destb.get_id(), [destb,v]{
        destb.ptr()->append(v);
      });
    }
  });
    
  scatter_time = walltime() - t;
  LOG_CHARM_LEVEL(INFO) << "scatter_time: " << scatter_time;
  t = walltime();

  // sort buckets locally
  // forall_local<bucket_t,sort_bucket>(bucketlist, nbuckets);
  /// Do some kind of local serial sort of a bucket
  pfor(bucketlist, nbuckets, [](int64_t bucket_id, bucket_t& bucket){
    if (bucket.size() == 0) return;
    qsort(&bucket[0], bucket.size(), sizeof(uint64_t), &ui64cmp);
  });

  local_sort_scatter_time = walltime() - t;
  LOG_CHARM_LEVEL(INFO) << "local_sort_time: " << local_sort_scatter_time;  
  t = walltime(); 
  ...
  pfor(bucketlist, nbuckets, [array](int64_t b, bucket_t& bucket) {
    ...
    //c.block_until_released();
    LOG_CHARM_LEVEL(3) << "bucket[" << b << "] release successful";
  });
  
  put_back_time = walltime() - t;
  LOG_CHARM_LEVEL(INFO) << "put_back_time: " << put_back_time;
  
  sort_time = walltime() - sort_time;
  LOG_CHARM_LEVEL(INFO) << "total_sort_time: " << sort_time;
}
...
int main(int argc, char* argv[]) {
  ...
  Charm::run([]{
    //all_do([]{ Metrics::reset(); });

    double t, rand_time;

    LOG_CHARM_LEVEL(INFO) << "### Sort Benchmark ###";
    LOG_CHARM_LEVEL(INFO) << "nelems = (1 << " << scale << ") = " << nelems << " (" << ((double)nelems)*sizeof(uint64_t)/(1L<<30) << " GB)";
    LOG_CHARM_LEVEL(INFO) << "nbuckets = (1 << " << log2buckets << ") = " << nbuckets;
    LOG_CHARM_LEVEL(INFO) << "maxkey = (1 << " << log2maxkey << ") - 1 = " << maxkey;

    //LOG_CHARM_LEVEL(INFO) << "iobufsize_mb = " << (double)BUFSIZE/(1L<<20);
//    LOG_CHARM_LEVEL(INFO) << "block_size = " << block_size;
    ...
    if (generate || write_to_disk || (read_from_disk && !fs::exists(dirname))) {
      LOG_CHARM_LEVEL(INFO) << "generating...";
      t = walltime();

      // fill vector with random 64-bit integers
      // forall_local<uint64_t,set_random>(array, nelems);
      pfor(array, nelems, [](int64_t i, uint64_t& e){ e = next_random(); });

      rand_time = walltime() - t;
      LOG_CHARM_LEVEL(INFO) << "fill_random_time: " << rand_time;
    ...
    if (read_from_disk) {
      memset(array, (uint64_t)0, nelems);
