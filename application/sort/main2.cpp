...
struct bucket_t {
  ...
  void reserve(size_t nelems) {
    ...
    if (!v) {
      LOG_CHARM(ERROR) << "Unable to allocate bucket of " << nelems << " elements."; exit(1);
    }
  }
...
void bucket_sort(GlobalAddress<uint64_t> array, size_t nelems, size_t nbuckets) {
  ...
  for (size_t i=0; i<nbuckets; i++) {
    ...
    VLOG_CHARM(1) << "bucket[" << i << "] on Core " << bi.core() << ", offset = " << bi.pointer() - bucketlist.localize(bi.core()) << ", ptr = " << bi.pointer();
  }
#endif
  ...
  LOG_CHARM(INFO) << "histogram_time: " << histogram_time;
  t = Grappa::walltime();
  ...
  on_all_cores([nbuckets]{
    ...
    VLOG_CHARM(1) << "after allreduce_inplace (just in case)";
  ...
  LOG_CHARM(INFO) << "allreduce_time: " << allreduce_time;

  // allocate space in buckets
  VLOG_CHARM(3) << "allocating space...";
  ...
  LOG_CHARM(INFO) << "scatter_time: " << scatter_time;
  t = Grappa::walltime();
  ...
  LOG_CHARM(INFO) << "local_sort_time: " << local_sort_scatter_time;  
  t = Grappa::walltime(); 
  ...
  forall(bucketlist, nbuckets, [array](int64_t b, bucket_t& bucket) {
    ...
    //c.block_until_released();
    VLOG_CHARM(3) << "bucket[" << b << "] release successful";
  });
  
  put_back_time = Grappa::walltime() - t;
  LOG_CHARM(INFO) << "put_back_time: " << put_back_time;
  
  sort_time = Grappa::walltime() - sort_time;
  LOG_CHARM(INFO) << "total_sort_time: " << sort_time;
}
...
int main(int argc, char* argv[]) {
  ...
  Grappa::run([]{
    ...
    LOG_CHARM(INFO) << "### Sort Benchmark ###";
    LOG_CHARM(INFO) << "nelems = (1 << " << scale << ") = " << nelems << " (" << ((double)nelems)*sizeof(uint64_t)/(1L<<30) << " GB)";
    LOG_CHARM(INFO) << "nbuckets = (1 << " << log2buckets << ") = " << nbuckets;
    LOG_CHARM(INFO) << "maxkey = (1 << " << log2maxkey << ") - 1 = " << maxkey;

    //LOG_CHARM(INFO) << "iobufsize_mb = " << (double)BUFSIZE/(1L<<20);
    LOG_CHARM(INFO) << "block_size = " << block_size;
    ...
    if (generate || write_to_disk || (read_from_disk && !fs::exists(dirname))) {
      ...
      LOG_CHARM(INFO) << "fill_random_time: " << rand_time;
